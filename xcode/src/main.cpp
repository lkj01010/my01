...
void func_test(){
	...
	if (g_val == 90){
    tasks_processor& s = singleton<tasks_processor>::instance();
		s.stop();
	}
}
//
...
//	}
	...
	{
		singleton<tasks_processor>::instance().push_task(&func_test);
		singleton<tasks_processor>::instance().push_task(
...
//	tasks_processor::get().start();
    singleton<tasks_processor>::instance().start();
	//assert(g_val == 90);
